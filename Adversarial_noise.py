...
x_noise=tf.Variable(tf.zeros([img_size,img_size,num_channels]),name='x_noise',
                    trainable=False,collections=collections)
x_noise_clip=tf.assign(x_noise,tf.clip_by_value(x_noise,-noise_limit,noise_limit))
x_noise_image=x_image+x_noise
x_noise_image=tf.clip_by_value(x_noise_image,0.0,1.0)

#use pretty tensor
x_pretty=pt.wrap(x_noise_image)
...
#         feed_dict_valid={x:data.validation.images,y_true:data.validation.labels}
#     #     if i%100==0:
...
#             break
def optimize(num_iterations,x_train,y_train,adversary_target_cls=None):
...
if True:
    for i in range(num_networks):
        ...
        optimize(num_iterations=num_iterations,
                 x_train=x_train,y_train=y_train,adversary_target_cls=3)
        print()
